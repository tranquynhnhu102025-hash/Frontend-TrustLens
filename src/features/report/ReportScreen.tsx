  ...
  return (
    <div className="w-full max-w-6xl mx-auto my-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-minimal-sm border border-slate-100 flex flex-col items-center justify-center text-center relative overflow-hidden">
          ...
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-amber-50 text-amber-700 font-bold text-xs border border-amber-100 mb-4">
            <AlertTriangle size={14} /> Mức độ: {report.level}
          </div>
          <p className="text-xs text-slate-500 font-bold leading-relaxed">{report.summary}</p>
        </div>

        {/* THẺ TIÊU CHÍ THÀNH PHẦN (CRITERIA BREAKDOWN) */}
        <div className="lg:col-span-2 bg-white p-8 rounded-2xl shadow-minimal-sm border border-slate-100">
              ...
              <div key={idx}>
                <div className="flex justify-between items-end mb-2">
                  <div>
                    ...
                    <span className="text-[10px] font-bold text-slate-400 ml-2 bg-slate-50 border border-slate-200/40 px-2 py-0.5 rounded-lg">Trọng số: {item.weight}</span>
                  </div>
                  <span className="font-black text-slate-800 text-sm">{item.score}/100</span>
                </div>
                <div className="w-full bg-slate-100 rounded-full h-2">
                  <div 
                    className={`h-2 rounded-full ${item.score >= 80 ? 'bg-green-500' : item.score >= 60 ? 'bg-amber-500' : 'bg-rose-500'}`} 
      ...
      <div className="bg-white rounded-2xl shadow-minimal-sm border border-slate-100 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 text-xs">
              {report.citations.map((cite: any) => (
                <tr key={cite.id} className="hover:bg-slate-50/20 transition-colors">
                  <td className="p-4 pl-6">
                    <p className="font-bold text-slate-800 line-clamp-2">{cite.title}</p>
                    <p className="font-medium text-slate-400 mt-1">{cite.author}</p>
                  </td>
                  <td className="p-4">
                    <p className="font-bold text-slate-700">{cite.year}</p>
                    <p className="font-medium text-slate-400 mt-1">{cite.source}</p>
                  </td>
                  <td className="p-4">
                    <div className="flex items-start gap-2">
                      {cite.status !== 'pass' && <Info size={14} className={`mt-0.5 shrink-0 ${cite.status === 'warning' ? 'text-amber-500' : 'text-rose-500'}`} />}
                      <span className={`font-medium ${cite.status === 'pass' ? 'text-slate-450' : 'text-slate-600'}`}>
                        {cite.issues}
                      </span>
                    </div>
                  </td>
                  <td className="p-4 text-center">
                    {cite.status === 'pass' && <span className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-green-50 text-green-700 font-bold rounded-lg border border-green-150"><CheckCircle2 size={12} /> Hợp lệ</span>}
                    {cite.status === 'warning' && <span className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-amber-50 text-amber-700 font-bold rounded-lg border border-amber-155"><AlertTriangle size={12} /> Chú ý</span>}
                    {cite.status === 'fail' && <span className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-rose-50 text-rose-700 font-bold rounded-lg border border-rose-155"><ShieldAlert size={12} /> Rủi ro cao</span>}
                  </td>
                </tr>
              ))}
            </tbody>
